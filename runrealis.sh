...
if [ $USER == 'root' ]
then
echo -e "                           \e[38;5;9mroot user is not supported for this installation!\e[39m"
...
ExecStart=$HOME/Realis.Network/target/release/realis --chain ./realis.json --ws-port 9944  --rpc-port 9933  --validator  --rpc-methods=Unsafe  --listen-addr /ip4/0.0.0.0/tcp/30333 --name $varname --unsafe-ws-external --unsafe-rpc-external --rpc-cors '*' -d ../realis/database
Restart=always
RestartSec=3
